...
describe("OperatorDashboardPage", () => {
  it("renders the operator work queues", () => {
    ...
    expect(
      screen.getByText(/Thời gian được tính từ lúc bàn bắt đầu chờ món/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("heading", { name: "Khiếu nại" }),
    ).toBeInTheDocument();
    expect(screen.getByText("3 phản ánh")).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "Mở đơn của Bàn 05" }),
    ).toHaveAttribute("href", "/operator/orders/ORD-0821");
    ...
    expect(
      screen.queryByRole("heading", { name: "Hoạt động gần đây" }),
    ...
    expect(
      within(complaintButton).getByText(/Khách phản ánh món mang ra còn thiếu/),
    ...
    expect(
      within(complaintDialog).getByText(
        /muốn nhân viên kiểm tra lại toàn bộ món trên bàn/i,
      ),
    ).toBeInTheDocument();

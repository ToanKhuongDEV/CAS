...
describe("OperatorOrderDetailPage", () => {
  it("renders the items, options, progress, note, and totals of one order", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Đơn của Bàn 03" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Bò làm chín vừa, mang khoai ra trước nếu xong."),
    ).toBeInTheDocument();
    expect(screen.getAllByText("275.000đ")).toHaveLength(2);
    expect(
      screen.getByRole("link", { name: "Quay lại danh sách" }),
    ).toHaveAttribute("href", "/operator/orders");
  });

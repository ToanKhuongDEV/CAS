...
import OperatorOrderDetailPage, { generateMetadata } from "../app/(operator)/operator/(workspace)/orders/[orderNumber]/page";
...
describe("OperatorOrderDetailPage", () => {
	it("renders the items, options, progress, note, and totals of one order", async () => {
		...
		expect(screen.getByRole("link", { name: "Quay lại danh sách" })).toHaveAttribute("href", "/operator/orders");
	});

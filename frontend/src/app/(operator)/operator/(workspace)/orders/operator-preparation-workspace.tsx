...
function buildTablePreparationGroups(preparationGroups: PreparationGroup[]): TablePreparationGroup[] {
	...
	return [...groupsByTable.values()].sort((first, second) => first.table.localeCompare(second.table, "vi", { numeric: true }));
}
	...
	return (
		<div className="overflow-hidden rounded-xl border border-cas-outline-variant/30 bg-cas-glass">
			{tableGroups.map((tableGroup, tableIndex) => (
				<details className="group border-b border-cas-outline-variant/25 last:border-b-0" key={tableGroup.table} open={tableIndex === 0}>
					<summary className="grid cursor-pointer list-none grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5 px-4 py-3.5 transition hover:bg-cas-primary/5 focus-visible:outline-3 focus-visible:outline-offset-[-3px] focus-visible:outline-cas-focus-ring [&::-webkit-details-marker]:hidden">
						<span className="text-lg leading-none font-bold text-cas-on-surface-variant transition-transform group-open:rotate-90" aria-hidden="true">
							›
						</span>
						<div className="min-w-0">
							<h3 className="truncate text-sm font-extrabold">{tableGroup.table}</h3>
							<p className="mt-0.5 truncate text-xs text-cas-on-surface-variant">{tableGroup.items.length} loại món</p>
						</div>
						<span className="shrink-0 rounded-lg bg-cas-secondary-container/25 px-2.5 py-1 text-xs font-extrabold text-cas-secondary">{tableGroup.totalRemainingQuantity} phần</span>
					</summary>
					...
					<ul className="ml-7 border-t border-l-2 border-cas-outline-variant/30 bg-cas-surface-container/35 py-1">
						{tableGroup.items.map((item) => (
							<li className="relative grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-cas-outline-variant/20 py-3 pr-4 pl-5 last:border-b-0" key={item.id}>
								...
								<span className="text-sm font-extrabold text-cas-primary">{item.remainingQuantity} phần</span>
							</li>
						))}
					</ul>
				</details>
...
export function OperatorPreparationWorkspace() {
	...
	const totalRemainingQuantity = preparationGroups.reduce((total, group) => total + group.remainingQuantity, 0);
	...
	function handlePreparedQuantitySubmit(event: FormEvent<HTMLFormElement>, groupId: string) {
		...
		if (!group || !Number.isInteger(preparedQuantity) || preparedQuantity < 1 || preparedQuantity > group.remainingQuantity) {
			setFeedback({
	...
	return (
		<>
			<header className="flex flex-wrap items-end justify-between gap-4">
				...
				<div className="flex items-center gap-3">
					...
					<div className="rounded-xl border border-cas-outline-variant/25 bg-cas-glass px-4 py-2.5 text-right">
						...
						<p className="mt-0.5 text-lg font-extrabold text-cas-primary">{totalRemainingQuantity} phần</p>
					</div>
				</div>
			</header>
			...
			{feedback ? (
				...
				>
					<CasIcon className="mt-0.5 size-5 shrink-0" name={feedback.tone === "success" ? "check" : "info"} />
			...
			<div className="mt-7 grid items-start gap-6 xl:grid-cols-2">
				<section aria-labelledby="preparation-groups-title">
					<div className="mb-3">
						...
						<p className="mt-1 text-xs text-cas-on-surface-variant">Các món cùng cấu hình được gộp thành một nhóm.</p>
					</div>
					...
					{preparationGroups.length === 0 ? (
						<div className="grid min-h-64 place-items-center rounded-xl border border-dashed border-cas-outline-variant/50 bg-cas-glass p-8 text-center">
							<div>
								...
								<p className="mt-1 text-sm text-cas-on-surface-variant">Hiện không còn món nào đang chờ chế biến.</p>
							</div>
						</div>
					) : (
						<div className="overflow-hidden rounded-xl border border-cas-outline-variant/30 bg-cas-glass">
							{preparationGroups.map((group, groupIndex) => (
								<details className="group border-b border-cas-outline-variant/25 last:border-b-0" key={group.id} open={groupIndex === 0}>
									<summary className="grid cursor-pointer list-none grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5 px-4 py-3.5 transition hover:bg-cas-primary/5 focus-visible:outline-3 focus-visible:outline-offset-[-3px] focus-visible:outline-cas-focus-ring [&::-webkit-details-marker]:hidden">
										<span className="text-lg leading-none font-bold text-cas-on-surface-variant transition-transform group-open:rotate-90" aria-hidden="true">
											›
										</span>
										...
										<span className="shrink-0 rounded-lg bg-cas-primary/8 px-2.5 py-1 text-xs font-extrabold text-cas-primary">{group.remainingQuantity} phần</span>
									</summary>
									...
									<div className="border-t border-cas-outline-variant/20 bg-cas-surface-container/35 px-4 py-3 sm:pl-14">
										<ol className="ml-3 border-l-2 border-cas-outline-variant/35">
											{group.allocations.map((allocation) => (
												<li className="relative grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-cas-outline-variant/20 py-3 pr-1 pl-5 last:border-b-0" key={allocation.id}>
													<span className="absolute top-1/2 -left-1.5 size-2.5 -translate-y-1/2 rounded-full border-2 border-cas-surface-container bg-cas-secondary" />
													<div>
														<Link
														...
														<p className="mt-0.5 text-xs text-cas-on-surface-variant">Gửi lúc {allocation.requestedAt}</p>
													</div>
													<p className="text-sm font-extrabold text-cas-primary">{allocation.remainingQuantity} phần</p>
												</li>
											))}
										</ol>

										<form className="mt-3 flex flex-wrap items-end gap-2 border-t border-cas-outline-variant/25 pt-3" onSubmit={(event) => handlePreparedQuantitySubmit(event, group.id)}>
											<label className="min-w-40 flex-1" htmlFor={`prepared-${group.id}`}>
												<span className="text-xs font-bold text-cas-on-surface-variant">Số phần vừa làm xong</span>
												<input
											...
											<button className="h-10 rounded-lg bg-cas-primary px-3.5 text-sm font-extrabold text-cas-on-primary transition hover:brightness-95 focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-cas-focus-ring active:translate-y-px" type="submit">
												Xác nhận hoàn thành
											</button>
										</form>
									</div>
				...
				<section aria-labelledby="table-preparation-title">
					<div className="mb-3">
						...
						<p className="mt-1 text-xs text-cas-on-surface-variant">Xem các món còn cần làm của từng bàn.</p>
					</div>

					{tablePreparationGroups.length > 0 ? (
						<TablePreparationTree tableGroups={tablePreparationGroups} />
					) : (
						<div className="grid min-h-32 place-items-center rounded-xl border border-dashed border-cas-outline-variant/50 bg-cas-glass p-6 text-center text-sm text-cas-on-surface-variant">Không còn bàn nào đang chờ món.</div>
					)}
				</section>
			</div>
		</>

	...
	return (
		<div className="mx-auto max-w-3xl">
			<header className="mb-8">
				<Link href="/operator/dashboard" className="mb-4 inline-flex items-center gap-1.5 text-sm font-bold text-cas-on-surface-variant transition hover:text-cas-primary hover:underline underline-offset-4">
					<CasIcon name="arrow" className="size-4 rotate-180" />
					Quay lại Bảng điều khiển
				</Link>
				<h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl text-cas-primary">Hủy món do sự cố</h1>
			</header>

			<form onSubmit={handleSubmit} className="space-y-8 rounded-3xl bg-cas-glass border border-cas-outline-variant/30 p-6 shadow-[0_5px_18px_var(--cas-shadow-color)] sm:p-8">
				<section className="space-y-5">
					<div className="flex items-center gap-2">
						<span className="grid size-7 shrink-0 place-items-center rounded-full bg-cas-primary/15 text-xs font-extrabold text-cas-primary">1</span>
					...
					{selectedTable ? (
						<div className="space-y-3 pt-2">
							<div className="flex items-center justify-between">
								<span className="text-xs font-extrabold tracking-wider text-cas-on-surface-variant uppercase">Món đã gọi tại {selectedTable}</span>
							</div>

							<div className="space-y-3">
								{currentItems.map((item) => {
									const cancelQty = cancelQuantities[item.id] ?? 0;
									const itemUnitPriceWithTopping = item.unitPrice + item.options.reduce((acc, opt) => acc + opt.price, 0);

									return (
										<div key={item.id} className={`rounded-2xl border p-4 transition-all duration-200 ${cancelQty > 0 ? "border-cas-primary bg-cas-primary/5 shadow-sm" : "border-cas-outline-variant/30 bg-cas-surface/80"}`}>
											<div className="flex items-start justify-between gap-4">
												<div className="min-w-0 flex-1">
													<div className="flex items-center gap-2">
														<span className="inline-flex size-6 shrink-0 items-center justify-center rounded-md bg-cas-primary/10 text-xs font-extrabold text-cas-primary">x{item.quantity}</span>
													...
													<div className="mt-2.5 space-y-1 text-xs text-cas-on-surface-variant">
														...
														{item.options.map((opt) => (
															<p key={opt.name} className="flex justify-between gap-3 text-xs">
																...
																<span className="font-semibold text-cas-on-surface">{opt.price > 0 ? `+${formatPrice(opt.price)}` : "Miễn phí"}</span>
															</p>
														))}
													</div>
													...
													<div className="mt-2 pt-2 border-t border-cas-outline-variant/15 flex items-center justify-between text-xs">
														...
														<strong className="font-extrabold text-cas-primary">{formatPrice(itemUnitPriceWithTopping * item.quantity)}</strong>
													</div>
												</div>

												{/* NÚT TRỪ / CỘNG BÊN PHẢI */}
												<div className="shrink-0 text-right">
													<span className="block text-[0.7rem] font-bold text-cas-on-surface-variant uppercase tracking-wider mb-1.5">Số lượng hủy</span>
													<div className="flex items-center rounded-xl border border-cas-outline-variant/45 bg-cas-surface overflow-hidden shadow-xs">
														...
														<span className="w-9 text-center text-base font-extrabold text-cas-primary">{cancelQty}</span>
														<button
															type="button"
															onClick={() => updateCancelQty(item.id, Math.min(item.quantity, cancelQty + 1))}
													...
													{cancelQty > 0 ? (
														...
														<span className="mt-1.5 block text-[0.72rem] text-cas-on-surface-variant">Chưa chọn hủy</span>
													)}
												</div>
											</div>
										</div>
						...
						<div className="rounded-2xl border border-dashed border-cas-outline-variant/40 bg-cas-surface/50 p-6 text-center text-xs text-cas-on-surface-variant">Vui lòng chọn bàn ở trên để xem chi tiết các món ăn và chọn số lượng cần hủy.</div>
					)}
				</section>

				{validationError ? <div className="rounded-xl bg-cas-error-container/20 border border-cas-error/30 p-3 text-xs font-bold text-cas-error">{validationError}</div> : null}

				<section className="space-y-5 border-t border-cas-outline-variant/25 pt-8">
					<div className="flex items-center gap-2">
						<span className="grid size-7 shrink-0 place-items-center rounded-full bg-cas-primary/15 text-xs font-extrabold text-cas-primary">2</span>
					...
					<div>
						...
						<div className="grid gap-3 sm:grid-cols-2">
							...
							>
								<span className={`mt-0.5 grid size-5 shrink-0 place-items-center rounded-full border-2 text-xs font-bold ${isRemade === true ? "border-cas-secondary bg-cas-secondary text-cas-on-primary" : "border-cas-outline-variant"}`}>{isRemade === true ? "✓" : ""}</span>
								<div>
									<strong className="block text-sm font-extrabold">Có</strong>
									<p className="mt-1 text-xs text-cas-on-surface-variant leading-relaxed">Tự động tạo phiếu chế biến mới gửi bếp làm lại món.</p>
								</div>
							</button>
							...
							>
								<span className={`mt-0.5 grid size-5 shrink-0 place-items-center rounded-full border-2 text-xs font-bold ${isRemade === false ? "border-cas-primary bg-cas-primary text-cas-on-primary" : "border-cas-outline-variant"}`}>{isRemade === false ? "✓" : ""}</span>
								<div>
									<strong className="block text-sm font-extrabold">Không</strong>
									<p className="mt-1 text-xs text-cas-on-surface-variant leading-relaxed">Trừ món khỏi đơn hàng và không làm lại.</p>
								</div>
							</button>
						</div>
					</div>
				...
				<div className="border-t border-cas-outline-variant/40 pt-8 text-right">
					<CasButton type="submit" size="lg" icon="trash" className="w-full sm:w-auto shadow-[0_8px_20px_var(--cas-shadow-color)]" disabled={isSubmitting}>

...
export default function AdminOperatorsPage() {
  ...
  const toggleLock = (id: number) => {
    setOperators((prev) =>
      prev.map((op) => (op.id === id ? { ...op, status: op.status === "ACTIVE" ? "LOCKED" : "ACTIVE" } : op))
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-cas-on-surface-variant">
            Tạo, kích hoạt hoặc khóa tài khoản nhân viên phục vụ/thu ngân qua Firebase Authentication.
          </p>
        </div>
        <CasButton
      ...
      {showAddForm && (
        <form onSubmit={handleAddOperator} className="rounded-3xl border border-cas-outline-variant/30 bg-cas-glass p-6 shadow-md space-y-4 max-w-lg">

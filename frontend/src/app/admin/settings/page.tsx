    ...
    <div className="relative">
      ...
      <div className="flex items-center gap-2.5 rounded-xl border border-cas-outline-variant/40 bg-cas-surface px-3.5 py-2.5 font-bold text-cas-on-surface shadow-xs hover:border-cas-primary/50 focus-within:ring-2 focus-within:ring-cas-primary transition">
        ...
        <button
          ...
          className="flex-1 text-left font-black text-sm tracking-wide text-cas-on-surface focus:outline-none cursor-pointer"
        >
          {displayTime}
        </button>
        <button
          ...
          className="shrink-0 rounded-lg bg-cas-primary/10 hover:bg-cas-primary hover:text-white px-2.5 py-1 font-black text-xs text-cas-primary transition border border-cas-primary/20 cursor-pointer"
          title="Bấm để chuyển đổi nhanh AM / PM"
        >
          {period}
        </button>
      </div>
        ...
        <div className="absolute top-full left-0 mt-2 z-50 w-64 rounded-2xl border border-cas-outline-variant/30 bg-cas-surface p-3.5 shadow-2xl space-y-3 animate-in fade-in zoom-in-95 duration-150">
          <div className="flex items-center justify-between border-b border-cas-outline-variant/20 pb-2">
            ...
            <button
              ...
              className="text-cas-on-surface-variant hover:text-cas-primary font-bold text-xs cursor-pointer"
            >
              ✕
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {/* Cột chọn Giờ */}
            <div>
              <span className="block text-[0.65rem] font-bold text-cas-on-surface-variant mb-1">
                Giờ
              </span>
                ...
                {Array.from({ length: 12 }, (_, i) => i + 1).map((n) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => setHour(n)}
                      className={`w-full text-center py-1 rounded-lg font-extrabold transition text-xs cursor-pointer ${isSelected ? "bg-cas-primary text-white shadow-xs" : "hover:bg-cas-surface-variant/60 text-cas-on-surface"}`}
                    >
                      {n < 10 ? `0${n}` : n}
                    </button>
                  );
          ...
          <div className="pt-2 border-t border-cas-outline-variant/15 flex items-center justify-between">
            <button
              type="button"
              onClick={togglePeriod}
              className="text-xs font-black text-cas-primary hover:underline flex items-center gap-1 cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => setIsOpen(false)}
              className="rounded-lg bg-cas-primary px-3 py-1 text-xs font-bold text-white hover:opacity-90 transition cursor-pointer"
            >
              Xác nhận
            </button>
          </div>
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-black text-cas-on-surface">
            Cấu hình Cửa hàng & Tham số Vận hành
          </h1>
          <p className="text-xs text-cas-on-surface-variant">
            Quản lý thông tin cửa hàng và tham số vận hành
          </p>
        </div>
        <div className="mt-2 sm:mt-0 flex items-center gap-2">
          ...
          >
            <span
              className={`h-2 w-2 rounded-full ${storeStatus === "ACTIVE" ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`}
            />
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-dashed border-cas-outline-variant/50 bg-cas-surface p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-cas-on-surface">
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1">
                  <label className="block font-extrabold text-cas-on-surface">
                    Gắn tọa độ vị trí quán bằng Google Maps
                  </label>

                  {/* Floating Tooltip Container (Hỗ trợ vừa Hover vừa Click, Không che/tối màn hình) */}
                  <div
                    className="relative inline-block"
                    onMouseEnter={() => setShowMapHelp(true)}
                    onMouseLeave={() => setShowMapHelp(false)}
                  >
                    <button
                      type="button"
                      ...
                      <div className="absolute top-full left-0 mt-2 z-40 w-72 sm:w-80 rounded-2xl border border-cas-outline-variant/30 bg-cas-surface p-4 shadow-2xl space-y-2.5 animate-in fade-in zoom-in-95 duration-150 text-xs">
                        <div className="flex items-center justify-between border-b border-cas-outline-variant/20 pb-2">
                          ...
                          <button
                            ...
                            className="text-cas-on-surface-variant hover:text-cas-primary font-bold text-xs"
                          >
                            ✕
                          </button>
                        </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a
                    href={
                      googleMapUrl
                        ? googleMapUrl.startsWith("http")
                          ? googleMapUrl
                          : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(googleMapUrl)}`
                ...
                <div className="flex items-center gap-4 mt-2">
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-cas-on-surface">
                    <input
                      ...
                      className="h-4 w-4 text-cas-primary accent-cas-primary focus:ring-cas-primary"
                    />
          ...
          <div className="rounded-3xl border border-cas-outline-variant/30 bg-cas-glass p-6 sm:p-8 shadow-xs">
            <div className="flex items-center gap-3 border-b border-cas-outline-variant/15 pb-4">
              ...
              <div>
                <h2 className="text-lg font-black text-cas-on-surface">
                  Tham số Vận hành & Cảnh báo
                </h2>
                <p className="text-xs text-cas-on-surface-variant">
                  Thiết lập quy tắc thời gian phục vụ món và ngưỡng cảnh báo quá tải cho nhân viên.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveOpsSettings} className="mt-6 space-y-5 text-xs">
              <div>
                ...
                <p className="mt-1 text-cas-on-surface-variant">
                  Nếu một bàn có order còn món chưa làm xong quá thời gian này, hệ thống sẽ bật cảnh
                  báo màu đỏ trên Dashboard Operator.
                </p>
                <div className="mt-3 flex items-center gap-3">
                  ...
                  />
                  <span className="font-extrabold text-cas-on-surface">
                    Phút (Mặc định chuẩn: 25 phút)
                  </span>
                </div>
              </div>
          ...
          <div className="rounded-3xl border border-cas-outline-variant/30 bg-cas-glass p-6 shadow-xs sticky top-6">
            ...
            <div className="mt-4 rounded-2xl border border-cas-outline-variant/20 bg-cas-surface p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="rounded-md bg-cas-primary/10 px-2 py-0.5 text-[0.65rem] font-black text-cas-primary">
                  CAS Store
                </span>
              </div>

              <div>
                <h4 className="text-base font-black text-cas-on-surface">
                  {storeName || "Tên Cửa hàng"}
                </h4>
                <p className="text-[0.7rem] text-cas-on-surface-variant mt-0.5">
                  {slogan || "Thông điệp chào mừng khách..."}
                </p>
              </div>
              ...
              <div className="space-y-1.5 pt-2 border-t border-cas-outline-variant/15 text-[0.75rem]">
                ...
                {googleMapUrl && (
                  <div className="flex items-start gap-2 text-cas-on-surface">
                    <CasIcon
                      name="sparkle"
                      className="h-3.5 w-3.5 shrink-0 text-cas-primary mt-0.5"
                    />
                    <a
                      href={
                        googleMapUrl.startsWith("http")
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-cas-primary underline font-bold hover:opacity-80"
                    >
            ...
            <div className="mt-4 rounded-2xl bg-cas-primary/5 p-3.5 border border-cas-primary/10">
              <p className="text-[0.7rem] text-cas-on-surface-variant leading-relaxed">
                <strong>Lưu ý:</strong> Mọi chỉnh sửa về tên quán, hotline, địa chỉ, email và vị trí
                Google Maps sẽ tự động đồng bộ trên hệ thống.
              </p>
            </div>
          </div>
        </div>

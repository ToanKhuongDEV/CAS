		...
		<div className="space-y-6">
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<p className="text-xs text-cas-on-surface-variant">Quản lý riêng các lựa chọn để liên kết với món ăn.</p>
				</div>
			...
			{showAddForm && (
				...
				>
					<form className="my-auto w-full max-w-md space-y-4 rounded-3xl border border-cas-outline-variant/30 bg-cas-surface p-6 shadow-2xl animate-in fade-in duration-150" onSubmit={addGroup}>
						...
						<div className="space-y-3 text-xs">
							<div>
								<label className="block font-bold text-cas-on-surface-variant">Tên nhóm option:</label>
								<input
			...
			<div className="grid gap-4 lg:grid-cols-2">
				{groups.map((group) => (
					<section className="rounded-3xl border border-cas-outline-variant/30 bg-cas-glass p-5" key={group.id}>
						<h2 className="font-black text-cas-on-surface">{group.name}</h2>
						<div className="mt-3 flex flex-wrap gap-2">
							{group.values.map((item) => (
								<span className="rounded-lg bg-cas-secondary/15 px-2.5 py-1 text-xs font-bold text-cas-secondary" key={item}>
									{item}
								</span>
							))}
						</div>
						<form
							...
							onSubmit={(event) => {
								...
								if (!next) return;
								setGroups((current) => current.map((item) => (item.id === group.id ? { ...item, values: [...item.values, next] } : item)));
								setValue((current) => ({ ...current, [group.id]: "" }));
							}}
						>
							<input className="min-w-0 flex-1 rounded-xl border border-cas-outline-variant/40 bg-cas-surface px-3 py-2 text-xs" onChange={(event) => setValue((current) => ({ ...current, [group.id]: event.target.value }))} placeholder="Giá trị option" value={value[group.id] ?? ""} />

...
export default function AdminTablesPage() {
	...
	const [statusFilter, setStatusFilter] = useState<"ALL" | "OPEN" | "CLOSED" | "PAYMENT_PENDING">("ALL");
	...
	const filteredTables = tables.filter((t) => {
		const matchesSearch = t.code.toString().includes(searchTerm.trim()) || t.qrToken.toLowerCase().includes(searchTerm.toLowerCase());
		...
		<div className="space-y-6 pb-12">
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<p className="text-xs text-cas-on-surface-variant">Danh sách mã bàn, mã QR token và trạng thái hoạt động trong nhà hàng.</p>
				</div>
			...
			<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-cas-outline-variant/30 bg-cas-glass p-3.5 shadow-xs backdrop-blur-md">
				<div className="relative flex-1 max-w-md">
					<CasIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 size-4 text-cas-on-surface-variant" name="search" />
					<input
				...
				<div className="flex items-center gap-1.5 overflow-x-auto text-xs">
					<button className={`rounded-xl px-3 py-1.5 font-bold transition ${statusFilter === "ALL" ? "bg-cas-primary text-cas-on-primary shadow-xs" : "bg-cas-surface text-cas-on-surface-variant hover:bg-cas-surface-container"}`} onClick={() => setStatusFilter("ALL")} type="button">
						Tất cả ({tables.length})
					</button>
					<button className={`rounded-xl px-3 py-1.5 font-bold transition ${statusFilter === "CLOSED" ? "bg-cas-primary text-cas-on-primary shadow-xs" : "bg-cas-surface text-cas-on-surface-variant hover:bg-cas-surface-container"}`} onClick={() => setStatusFilter("CLOSED")} type="button">
						Bàn trống ({tables.filter((t) => t.sessionStatus === "CLOSED").length})
					</button>
					<button className={`rounded-xl px-3 py-1.5 font-bold transition ${statusFilter === "OPEN" ? "bg-emerald-600 text-white shadow-xs" : "bg-cas-surface text-cas-on-surface-variant hover:bg-cas-surface-container"}`} onClick={() => setStatusFilter("OPEN")} type="button">
						Đang có khách ({tables.filter((t) => t.sessionStatus === "OPEN").length})
					</button>
					<button className={`rounded-xl px-3 py-1.5 font-bold transition ${statusFilter === "PAYMENT_PENDING" ? "bg-amber-600 text-white shadow-xs" : "bg-cas-surface text-cas-on-surface-variant hover:bg-cas-surface-container"}`} onClick={() => setStatusFilter("PAYMENT_PENDING")} type="button">
					...
					<table className="w-full text-left text-xs">
						...
						<tbody className="divide-y divide-cas-outline-variant/15">
							{filteredTables.length === 0 ? (
								<tr>
									<td className="px-5 py-8 text-center text-cas-on-surface-variant font-bold" colSpan={4}>
								...
								filteredTables.map((table) => (
									<tr className="transition hover:bg-cas-primary/5" key={table.id}>
										...
										<td className="px-5 py-4 font-mono font-bold text-cas-on-surface-variant text-[0.75rem]">{table.qrToken}</td>
										...
										<td className="px-5 py-4">
											...
											>
												<span className={`size-1.5 rounded-full ${table.sessionStatus === "OPEN" ? "bg-emerald-500" : table.sessionStatus === "PAYMENT_PENDING" ? "bg-amber-500 animate-ping" : "bg-cas-on-surface-variant/40"}`} />
										...
										<td className="px-5 py-4 text-right">
											<div className="flex items-center justify-end gap-2">
												<CasButton icon="search" onClick={() => setSelectedQR(table)} size="sm" variant="outline-primary">
													Xem QR
												</CasButton>
												<button className="grid size-8 place-items-center rounded-xl bg-cas-error/10 text-cas-error transition hover:bg-cas-error hover:text-white" onClick={() => setDeletingTable(table)} title={`Xóa Bàn ${table.code}`} type="button">
				...
				<div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/55 p-4 backdrop-blur-sm">
					<div className="w-full max-w-sm space-y-4 rounded-3xl border border-cas-outline-variant/30 bg-cas-surface p-6 shadow-2xl animate-in fade-in duration-150">
						<div className="flex items-center gap-3 text-cas-error">
							...
							<div>
								<h3 className="text-base font-black text-cas-on-surface">Xác nhận xóa BÀN {deletingTable.code}?</h3>
								<p className="text-xs text-cas-on-surface-variant">Thao tác này sẽ gỡ mã QR token {deletingTable.qrToken} khỏi hệ thống.</p>
							</div>
						</div>
						...
						<div className="flex justify-end gap-2 pt-2">
							...
							<CasButton onClick={() => handleDeleteTable(deletingTable.id)} size="sm" variant="danger">
								Xóa Bàn
							</CasButton>
						</div>
					</div>
				...
				<div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/55 p-4 backdrop-blur-sm">
					<div className="w-full max-w-sm space-y-4 rounded-3xl border border-cas-outline-variant/30 bg-cas-surface p-6 text-center shadow-2xl animate-in fade-in duration-150">
						<div className="flex items-center justify-between border-b border-cas-outline-variant/20 pb-3">
							...
							<button className="text-xs font-bold text-cas-on-surface-variant hover:text-cas-primary" onClick={() => setSelectedQR(null)} type="button">
								Đóng
							</button>
						</div>
						...
						<div className="mx-auto grid size-48 place-items-center rounded-2xl border border-cas-outline-variant/30 bg-white p-4 shadow-inner">
							<div className="grid size-36 place-items-center rounded-xl border-4 border-dashed border-cas-primary/40 text-center font-mono text-[0.68rem] font-black text-cas-primary leading-tight">
								[ QR CODE TOKEN ]<br />
								<span className="mt-1 text-[0.65rem] text-cas-on-surface-variant">{selectedQR.qrToken}</span>
							</div>
						</div>

  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-black text-cas-on-surface">Quản lý Mã giảm giá & Ưu đãi (Vouchers)</h1>
          <p className="text-xs font-medium text-cas-on-surface-variant">
            Tạo, cấu hình hạn mức và quản lý các chương trình mã ưu đãi giảm giá áp dụng cho đơn gọi món.
          </p>
        </div>
        <CasButton
      ...
      {showAddForm && (
        <form onSubmit={handleAddVoucher} className="rounded-3xl border border-cas-outline-variant/30 bg-cas-glass p-6 shadow-md space-y-4 max-w-2xl animate-in fade-in duration-150">
      ...
      {/* Danh sách Vouchers dạng Grid Cards */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filteredVouchers.map((voucher) => {
          ...
          return (
            <div
              key={voucher.id}
              className={`relative overflow-hidden rounded-3xl border border-cas-outline-variant/30 bg-cas-glass p-5 shadow-xs transition hover:shadow-md ${
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[0.65rem] font-black ${
              ...
              <div className="mt-4 space-y-1.5 border-t border-cas-outline-variant/20 pt-3 text-xs">
                ...
                <div className="flex justify-between text-[0.7rem] text-cas-on-surface-variant">
                  <span>Thời hạn:</span>
                  <span>{voucher.startAt} đến {voucher.endAt}</span>
                </div>
              </div>

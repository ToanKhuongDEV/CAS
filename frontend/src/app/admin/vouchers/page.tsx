    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs font-medium text-cas-on-surface-variant">
            Tạo mã ưu đãi voucher, cấu hình số lượt sử dụng và điều kiện áp dụng cho khách hàng.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 text-xs sm:grid-cols-2">
              ...
              <div>
                <label className="block font-bold text-cas-on-surface-variant">
                  {discountType === "FIXED_AMOUNT" ? "Số tiền giảm (VNĐ):" : "Tỷ lệ giảm (%):"}
                </label>
                <input
                  type="number"
                  value={discountValue}
      ...
      <div className="flex items-center gap-2">
        ...
        {(["ALL", "ACTIVE", "INACTIVE"] as const).map((st) => (
          <button
            ...
            className={`rounded-xl px-3 py-1 text-xs font-bold transition ${statusFilter === st ? "bg-cas-primary text-white font-black" : "bg-cas-glass text-cas-on-surface-variant hover:text-cas-on-surface"}`}
          >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filteredVouchers.map((voucher) => {
          const isExpired =
            voucher.status === "INACTIVE" || voucher.usageCount >= voucher.usageLimit;

          return (
            <div
              key={voucher.id}
              className={`relative overflow-hidden rounded-3xl border border-cas-outline-variant/30 bg-cas-glass p-5 shadow-xs transition hover:shadow-md ${isExpired ? "opacity-75" : ""}`}
            >
              <div className="flex items-start justify-between">
                <div>
                  <span className="inline-block rounded-lg bg-cas-primary/10 px-2.5 py-1 text-xs font-black tracking-wider text-cas-primary uppercase">
                    {voucher.code}
                  </span>
                  <p className="mt-2 text-xs font-medium text-cas-on-surface-variant">
                    {voucher.description || "Ưu đãi áp dụng đơn gọi món"}
                  </p>
                </div>
                <span
                  className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[0.65rem] font-black ${voucher.status === "ACTIVE" ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : "bg-gray-500/15 text-gray-500"}`}
                >
                ...
                <div className="flex justify-between font-bold">
                  ...
                  <span className="font-black text-cas-on-surface">
                    {voucher.discountType === "FIXED_AMOUNT"
                      ...
                      : `${voucher.discountValue}% ${voucher.maxDiscountAmount ? `(Tối đa ${voucher.maxDiscountAmount.toLocaleString("vi-VN")} đ)` : ""}`}
                  </span>
                </div>
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  className={`rounded-xl px-3 py-1.5 text-xs font-black transition ${voucher.status === "ACTIVE" ? "bg-rose-500/10 text-rose-600 hover:bg-rose-500/20" : "bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20"}`}
                >

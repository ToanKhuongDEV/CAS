...
export function PaymentRequestPanel({ confirmedAt = "20:05", paymentStatus = "NONE", tableQrToken }: PaymentRequestPanelProps) {
			...
			<div className="fixed inset-0 z-100 overflow-y-auto bg-cas-surface text-cas-on-surface">
				<main className="grid min-h-full place-items-center px-5 py-10">
					<section className="w-full max-w-md text-center" aria-labelledby="payment-success-title">
						...
						<p className="mt-7 text-xs font-extrabold tracking-[0.12em] text-cas-secondary uppercase">Đã hoàn tất</p>
						<h1 className="mt-2 text-3xl font-extrabold tracking-tight" id="payment-success-title">
							Thanh toán thành công
						</h1>
						<p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-cas-on-surface-variant">Cảm ơn bạn đã sử dụng dịch vụ tại CAS.</p>
						...
						<CasButton className="mt-6 w-full shadow-[0_8px_20px_var(--cas-shadow-color)]" size="lg" onClick={handleCreateNewOrder}>
	...
	return (
		<>
			<section className="rounded-[1.4rem] bg-cas-surface-container p-5 shadow-[0_10px_28px_var(--cas-shadow-color)] md:p-7" aria-labelledby="payment-summary-title">
				<div className="flex items-center justify-between gap-4 border-b border-cas-outline-variant/40 pb-4">
					<div>
						<p className="text-[0.65rem] font-extrabold tracking-[0.12em] text-cas-on-surface-variant uppercase">Phiên bàn hiện tại</p>
						<h2 className="mt-1 text-lg font-extrabold" id="payment-summary-title">
							Chi tiết thanh toán
						</h2>
					</div>
					<span className="rounded-full bg-cas-secondary-container/20 px-3 py-1 text-xs font-extrabold text-cas-secondary">4 món</span>
				</div>

				<ul className="divide-y divide-cas-outline-variant/35">
					{billItems.map((item) => (
						<li className="flex items-start gap-3 py-4" key={item.name}>
							<span className="grid size-8 shrink-0 place-items-center rounded-full bg-cas-surface text-xs font-extrabold text-cas-primary">{item.quantity}×</span>
							<div className="min-w-0 flex-1">
								<h3 className="text-sm font-extrabold">{item.name}</h3>
								<p className="mt-1 text-[0.68rem] leading-relaxed text-cas-on-surface-variant">{item.options}</p>
				...
				<div className="border-t border-cas-outline-variant/40 pt-4 text-sm">
					<div className="flex items-end justify-between gap-4">
						<div>
							<p className="text-[0.65rem] font-extrabold tracking-[0.12em] text-cas-on-surface-variant uppercase">Tổng cần thanh toán</p>
							<p className="mt-1 text-xs text-cas-on-surface-variant">Toàn bộ order của phiên bàn</p>
						</div>
			...
			<section className="mt-4 rounded-2xl border border-cas-secondary/20 bg-cas-secondary-container/20 p-4">
				<div className="flex items-start gap-3">
					...
					<div>
						...
						<p className="mt-1 text-xs leading-relaxed text-cas-on-surface-variant">Yêu cầu thanh toán áp dụng cho toàn bộ order trong phiên bàn và bạn sẽ không thể gọi thêm món sau khi gửi.</p>
					</div>
				</div>
			</section>

			<div className="mt-5">
				<CasButton className="w-full shadow-[0_8px_20px_var(--cas-shadow-color)]" size="lg" disabled={isWaitingForConfirmation} onClick={() => setIsPaymentPending(true)} icon={isWaitingForConfirmation ? "clock" : "payment"}>
					{isWaitingForConfirmation ? "Đã gửi yêu cầu thanh toán" : "Gửi yêu cầu thanh toán"}
				</CasButton>
				<p className="mt-3 text-center text-[0.68rem] leading-relaxed text-cas-on-surface-variant">Tổng tiền sẽ được hệ thống xác nhận lại khi gửi yêu cầu.</p>
			</div>

			{isWaitingForConfirmation ? (
				<div className="fixed inset-0 z-100 grid place-items-center bg-cas-on-surface/55 px-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="payment-pending-title" aria-describedby="payment-pending-description">
					<section className="w-full max-w-sm rounded-[1.4rem] bg-cas-surface-container p-6 text-center shadow-[0_16px_36px_var(--cas-shadow-color)]">
						...
						<p className="mt-3 text-sm leading-relaxed text-cas-on-surface-variant" id="payment-pending-description">

...
const variantClasses: Record<CasButtonVariant, string> = {
	...
	"outline-primary": "border border-cas-primary/30 bg-cas-surface-container text-cas-primary hover:bg-cas-primary/8",
...
export const CasButton = forwardRef<HTMLButtonElement | HTMLAnchorElement, CasButtonProps>(({ variant = "primary", size = "md", href, className = "", icon, children, ...props }, ref) => {
	const baseClasses = "inline-flex items-center justify-center gap-2 rounded-xl font-extrabold transition focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-cas-focus-ring active:translate-y-px disabled:opacity-50 disabled:pointer-events-none";
	...
	const renderIcon = () => {
		...
		if (typeof icon === "string") {
			return <CasIcon name={icon as CasIconName} className={size === "sm" ? "size-4" : "size-5"} />;
		}
		return icon;
	};

...
export function OperatorHeaderNotifications() {
  ...
  const [notifications, setNotifications] =
    useState<OperatorNotification[]>(mockOperatorNotifications);
        ...
        <div className="absolute right-0 top-full mt-2 w-80 overflow-hidden rounded-3xl border border-cas-outline-variant/30 bg-cas-surface p-4 shadow-2xl backdrop-blur-2xl animate-in fade-in duration-150 z-100">
          ...
          <div className="flex items-center justify-between border-b border-cas-outline-variant/20 pb-2.5">
            ...
            {unreadCount > 0 ? (
              ...
              <span className="text-[0.65rem] font-medium text-cas-on-surface-variant/70">
                Đã đọc hết
              </span>
            )}
          </div>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      {!n.isRead && (
                        <span
                          className="size-2 shrink-0 rounded-full bg-cas-primary animate-pulse"
                          title="Chưa đọc"
                        />
                      )}
                      <span
                        className={`font-extrabold ${n.isRead ? "text-cas-on-surface-variant" : "text-cas-on-surface"}`}
                      >
                        {n.title}
                      </span>
                    </div>
                    ...
                    <span
                      className={`shrink-0 rounded-md px-1.5 py-0.5 text-[0.6rem] font-black uppercase ${n.type === "URGENT" ? "bg-rose-500/20 text-rose-600" : n.type === "WARNING" ? "bg-amber-500/20 text-amber-600" : "bg-sky-500/20 text-sky-600"}`}
                    >
                      {n.type}
                    </span>
                  </div>
                  ...
                  <div className="mt-2 flex items-center justify-between text-[0.65rem]">
                    ...
                    <span className="text-[0.65rem] font-bold text-cas-primary opacity-0 transition group-hover:opacity-100">

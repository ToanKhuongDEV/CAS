...
type OperatorNavigationItem = {
  href: string;
  icon: CasIconName;
  id: "dashboard" | "payments" | "cancellations";
  label: string;
};

const navigationItems: OperatorNavigationItem[] = [
  { id: "dashboard", label: "Tổng quan", icon: "table", href: "/operator" },
  {
    id: "payments",
    label: "Thanh toán",
    icon: "payment",
    href: "#pending-payments-title",
  },
  {
    id: "cancellations",
    label: "Hủy món",
    icon: "minus",
    href: "#cancellation-requests-title",
  },
];

export function OperatorBottomNavigation() {
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-50 grid min-h-20 grid-cols-3 rounded-t-xl border-t border-cas-outline-variant/30 bg-cas-navigation px-3 pt-2 pb-[max(0.55rem,env(safe-area-inset-bottom))] shadow-[0_-4px_20px_var(--cas-shadow-color)] backdrop-blur-xl lg:hidden"

...
export function OperatorCancellationRequestsView() {
	...
	return (
		<>
			...
			{feedbackMessage ? (
				<div className="mt-6 flex items-start gap-3 rounded-2xl border border-cas-secondary/25 bg-cas-secondary-container/20 p-4 text-sm font-bold text-cas-secondary" role="status">
					<CasIcon className="mt-0.5 size-5 shrink-0" name="check" />
					<p className="flex-1 leading-relaxed">{feedbackMessage}</p>
					<button onClick={() => setFeedbackMessage(null)} className="text-xs opacity-75 hover:opacity-100" type="button">
						✕
					</button>
				</div>
			) : null}
			...
			{pendingRequests.length > 0 ? (
				<ul className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
					{pendingRequests.map((request) => (
						<li className="flex flex-col justify-between rounded-2xl border border-cas-outline-variant/25 bg-cas-glass p-5 shadow-[0_5px_18px_var(--cas-shadow-color)]" key={request.id}>
							<div>
								<div className="flex items-start justify-between gap-4">
									<div>
										<strong className="text-base font-extrabold">{request.table}</strong>
										<p className="mt-0.5 text-xs text-cas-on-surface-variant">Gửi lúc {request.requestedAt}</p>
									</div>
									<span className="rounded-full bg-cas-primary/10 px-3 py-1 text-xs font-extrabold text-cas-primary">Chờ xác nhận</span>
								</div>
								...
								<div className="mt-4 rounded-xl border border-cas-outline-variant/20 bg-cas-surface/60 p-3.5">
									...
									<p className="mt-1 text-xs font-bold text-cas-primary">Số lượng yêu cầu: {request.quantity}</p>
									{request.reason ? (
										<p className="mt-2 text-xs leading-relaxed text-cas-on-surface-variant">
											<span className="font-semibold text-cas-on-surface">Lý do:</span> {request.reason}
										</p>
									) : null}
								</div>
							</div>
							...
							<div className="mt-5 flex items-center gap-3">
								<CasButton variant="primary" size="sm" className="flex-1" onClick={() => openApproveModal(request)}>
									Đồng ý hủy
								</CasButton>
								<CasButton variant="outline" size="sm" className="flex-1" onClick={() => openRejectModal(request)}>
									Từ chối
								</CasButton>
							</div>
						</li>
				...
				<div className="mt-8 grid min-h-60 place-items-center rounded-2xl border border-dashed border-cas-outline-variant/40 bg-cas-glass p-8 text-center">
					<div>
						...
						<p className="mt-1 text-sm text-cas-on-surface-variant">Tất cả các yêu cầu từ phía bàn đã được xử lý xong.</p>
					</div>
				</div>
			)}
			...
			{activeModal?.type === "APPROVE" ? (
				...
				>
					<section className="w-full max-w-md rounded-3xl border border-cas-outline-variant/30 bg-cas-surface p-6 shadow-2xl" aria-labelledby="approve-dialog-title" role="dialog" aria-modal="true">
						<div className="flex items-start justify-between gap-4">
							<div>
								<span className="text-xs font-extrabold tracking-wider text-cas-primary uppercase">Xác nhận hệ thống</span>
								<h2 className="mt-1 text-xl font-extrabold" id="approve-dialog-title">
									Đồng ý hủy món?
								</h2>
							</div>
							<button onClick={() => setActiveModal(null)} className="grid size-9 place-items-center rounded-xl border border-cas-outline-variant/30 text-cas-on-surface-variant hover:text-cas-primary" type="button">
						...
						<div className="mt-4 rounded-2xl border border-cas-outline-variant/25 bg-cas-surface-container/60 p-4">
							...
							<div className="mt-2 flex justify-between text-sm">
								...
								<strong className="font-extrabold text-cas-primary">{activeModal.request.item}</strong>
							</div>
							<div className="mt-2 flex justify-between text-sm">
							...
							<div>
								...
								<div className="grid grid-cols-2 gap-3">
									...
									>
										...
										<span className="mt-0.5 text-[0.7rem] leading-tight opacity-80">Chế biến suất đền bù mới</span>
									</button>
									<button
										type="button"
										onClick={() => {
									...
									>
										...
										<span className="mt-0.5 text-[0.7rem] leading-tight opacity-80">Trừ khỏi đơn, không làm lại</span>
									</button>
								</div>
								{modalError ? <p className="mt-1.5 text-xs font-bold text-cas-error">{modalError}</p> : null}
							</div>

							<label className="block text-xs font-bold">
								<span className="mb-1.5 block text-cas-on-surface-variant">Ghi chú xử lý của nhân viên (không bắt buộc)</span>
								<input
									type="text"
									value={staffNote}
			...
			{activeModal?.type === "REJECT" ? (
				...
				>
					<section className="w-full max-w-md rounded-3xl border border-cas-outline-variant/30 bg-cas-surface p-6 shadow-2xl" aria-labelledby="reject-dialog-title" role="dialog" aria-modal="true">
						<div className="flex items-start justify-between gap-4">
							<div>
								<span className="text-xs font-extrabold tracking-wider text-cas-error uppercase">Từ chối yêu cầu</span>
						...
						<div className="mt-4 rounded-2xl border border-cas-outline-variant/25 bg-cas-surface-container/60 p-4">
							...
							<p className="mt-1 text-xs text-cas-on-surface-variant">
								Món: <strong className="text-cas-on-surface">{activeModal.request.item}</strong> ({activeModal.request.quantity})
							</p>
						</div>

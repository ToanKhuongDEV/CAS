...
import { AddToCartOptionDialog, type AddToCartPayload, type MenuOptionGroup } from "../../customer/add-to-cart-option-dialog";
...
const menuItems: MenuItemData[] = [
	{
		...
		description: "Sợi mỳ dai ngon quyện cùng nước dùng chua cay đậm đà, rau nấm và topping hấp dẫn.",
...
export function OperatorOrderCreationView({ defaultTableId = "table-05" }: OperatorOrderCreationViewProps) {
	...
	const totalCartAmount = useMemo(() => cartItems.reduce((acc, item) => acc + item.unitPrice * item.quantity, 0), [cartItems]);
	...
	return (
		<div className="relative pb-24 lg:pb-0">
			...
			<header className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
				<div>
					<h1 className="text-2xl font-extrabold tracking-tight text-cas-on-surface sm:text-3xl">Tạo order hộ tại bàn</h1>
				</div>
				...
				<div className="flex items-center gap-3 rounded-2xl border border-cas-outline-variant/30 bg-cas-surface p-2.5 shadow-sm sm:p-3">
					<span className="grid size-9 place-items-center rounded-xl bg-cas-secondary text-xs font-black text-cas-on-primary">{selectedTable.code}</span>
					<div className="pr-2">
						<p className="text-xs font-extrabold text-cas-on-surface">
							{selectedTable.label}
							{selectedTable.customerName ? ` (${selectedTable.customerName})` : ""}
						</p>
						<p className="text-[0.68rem] font-semibold text-emerald-600 dark:text-emerald-400">Phiên bàn đang mở</p>
					</div>
					<button type="button" onClick={() => setIsTableModalOpen(true)} className="rounded-xl bg-cas-secondary-container/30 px-3 py-1.5 text-xs font-extrabold text-cas-secondary transition hover:bg-cas-secondary-container/50 focus-visible:outline-2 focus-visible:outline-cas-focus-ring">
						Chọn bàn khác
					</button>
				</div>
			</header>
			...
			<div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
				...
				<div className="min-w-0 space-y-4 lg:col-span-7 xl:col-span-8">
					...
					<CategoryNavigation categories={categories} className="w-full max-w-full rounded-2xl border border-cas-outline-variant/30 px-2 py-2.5 mt-2 mx-0 shadow-sm" />
					...
					<div className="mt-2" id="menu-list">
						{categories.map((category) => {
							...
							return (
								<section className="scroll-mt-32 border-t border-cas-outline-variant/55 py-5 first:border-t-0 md:py-6" id={category.id} key={category.id} aria-labelledby={`${category.id}-title`}>
									<div className="mb-4 flex items-center justify-between gap-4">
										<h2 className="text-xl font-extrabold" id={`${category.id}-title`}>
											{category.label}
										</h2>
										<span className="text-xs font-semibold text-cas-on-surface-variant">{categoryItems.length} món</span>
									</div>

									<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
										{categoryItems.map((item, itemIndex) => (
											<article className="grid grid-cols-[7.25rem_1fr] gap-4 rounded-[1.2rem] bg-cas-surface-container p-3 shadow-[0_5px_18px_var(--cas-shadow-color)] md:p-4" key={item.id}>
												<div className="relative min-h-32 overflow-hidden rounded-2xl md:aspect-[4/3] md:min-h-0">
													<Image className="object-cover transition-transform duration-500 hover:scale-105" src={item.imageSrc} alt={item.imageAlt} fill loading={category.id === "mon-noi-bat" && itemIndex === 0 ? "eager" : "lazy"} sizes="(max-width: 767px) 7.25rem, (max-width: 1023px) 45vw, 30vw" />
													{item.badges.length > 0 ? (
														<div className="absolute top-2 left-2 flex flex-wrap items-start gap-1">
															{item.badges.map((badge, index) => (
																<span className={`rounded-full px-2 py-1 text-[0.55rem] font-extrabold text-white ${index === 0 ? "bg-cas-primary" : "bg-rose-600"}`} key={badge}>
																	{badge}
																</span>
															))}
														</div>
												...
												<div className="flex min-w-0 flex-col">
													<h3 className="line-clamp-2 text-sm leading-snug font-extrabold md:text-base">{item.name}</h3>
													<p className="mt-1.5 line-clamp-3 text-[0.7rem] leading-relaxed text-cas-on-surface-variant md:text-xs">{item.description}</p>
													<div className="mt-auto flex items-end justify-between gap-3 pt-3">
														<strong className="text-sm text-cas-primary md:text-base">{item.price}</strong>
														<AddToCartOptionDialog basePrice={item.basePrice} itemName={item.name} optionGroups={item.optionGroups} onAddToCart={(payload) => handleAddItemToCart(item, payload)} />
													</div>
												</div>
											</article>
										))}
			...
			{successOrderData && (
				<div className="fixed inset-0 z-100 flex items-center justify-center bg-cas-on-surface/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
					<div className="w-full max-w-md rounded-[1.6rem] bg-cas-surface p-6 shadow-2xl">
						<div className="flex flex-col items-center text-center">
							...
							<h3 className="mt-4 text-xl font-extrabold text-cas-on-surface">Tạo order thành công!</h3>
							<p className="mt-1 text-xs text-cas-on-surface-variant">Đơn đã được thêm vào hàng đợi chế biến FIFO của Bếp</p>
						</div>
						...
						<div className="mt-5 space-y-2 rounded-2xl border border-cas-outline-variant/30 bg-cas-surface-container p-4 text-xs">
							<div className="flex justify-between">
								...
								<span className="font-mono font-bold text-cas-on-surface">{successOrderData.orderNumber}</span>
							</div>
							<div className="flex justify-between">
							...
							<div className="flex justify-between">
								...
								<span className="font-bold text-cas-on-surface">{successOrderData.itemCount} món</span>
							</div>
							<div className="flex justify-between">
							...
							{successOrderData.orderNote && (
								<div className="border-t border-cas-outline-variant/20 pt-2 text-left">
									...
									<span className="font-medium text-cas-on-surface">&ldquo;{successOrderData.orderNote}&rdquo;</span>
								</div>
							)}
							<div className="flex justify-between border-t border-cas-outline-variant/20 pt-2 text-sm font-extrabold">
								<span>Tổng tiền:</span>
								<span className="text-cas-primary">{formatPrice(successOrderData.totalAmount)}</span>
							</div>
						</div>

						<button type="button" onClick={() => setSuccessOrderData(null)} className="mt-5 w-full rounded-xl bg-cas-primary py-3 text-center text-sm font-extrabold text-cas-on-primary shadow-md transition hover:bg-cas-primary-hover focus-visible:outline-2 focus-visible:outline-cas-focus-ring">
							Tiếp tục tạo order
						</button>
					</div>
				</div>

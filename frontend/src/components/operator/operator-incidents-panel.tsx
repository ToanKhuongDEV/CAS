...
const initialIncidents: OperationalIncident[] = [
	{
		...
		description: "Máy in bếp 1 bị kẹt giấy trong 10 phút, đã xử lý xong và in lại các phiếu order bị hoãn.",
		id: "inc-1",
	},
	{
...
export function OperatorIncidentsPanel() {
	...
	return (
		<>
			<section className="rounded-2xl border border-cas-outline-variant/20 bg-cas-glass p-5 shadow-[0_5px_18px_var(--cas-shadow-color)]" aria-labelledby="incidents-title">
				<div className="flex flex-wrap items-center justify-between gap-4">
					<div className="flex items-center gap-2.5">
						<h2 className="text-xl font-extrabold" id="incidents-title">
							Sự cố phát sinh
						</h2>
						<span className="rounded-full bg-cas-error/10 px-2.5 py-0.5 text-xs font-extrabold text-cas-error">{incidents.length} sự cố</span>
					</div>
				...
				{feedback ? (
					<div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-cas-secondary/25 bg-cas-secondary-container/20 p-3 text-xs font-bold text-cas-secondary" role="status">
						<div className="flex items-center gap-2">
							<CasIcon className="size-4 shrink-0" name="check" />
							<span>{feedback}</span>
						</div>
						<button onClick={() => setFeedback(null)} className="opacity-75 hover:opacity-100" type="button">
							✕
						</button>
					</div>
				) : null}
						...
						{incidents.map((inc) => (
							<li className="py-3 first:pt-1 last:pb-0" key={inc.id}>
								<div className="flex items-start justify-between gap-3">
									<span className="text-xs font-extrabold text-cas-on-surface">{inc.createdByName}</span>
									<time className="text-[0.7rem] text-cas-on-surface-variant shrink-0">{inc.createdAt}</time>
								</div>
								<p className="mt-1.5 text-xs leading-relaxed text-cas-on-surface-variant/90">{inc.description}</p>
							</li>
						))}
					</ul>
				) : (
					<p className="mt-4 text-center text-xs text-cas-on-surface-variant py-4">Chưa có sự cố nào được ghi nhận trong ca.</p>
				)}
			</section>
			...
			{isModalOpen ? (
				...
				>
					<section className="w-full max-w-lg rounded-3xl border border-cas-outline-variant/30 bg-cas-surface p-6 shadow-2xl" aria-labelledby="create-incident-title" role="dialog" aria-modal="true">
						<div className="flex items-start justify-between gap-4">
							<div>
								<span className="text-xs font-extrabold tracking-wider text-cas-error uppercase">Vận hành ca trực</span>
								<h2 className="mt-1 text-xl font-extrabold" id="create-incident-title">
									Báo cáo sự cố phát sinh
								</h2>
							</div>
							<button onClick={() => setIsModalOpen(false)} className="grid size-9 place-items-center rounded-xl border border-cas-outline-variant/30 text-cas-on-surface-variant hover:text-cas-primary" type="button">
						...
						<form onSubmit={handleSubmit} className="mt-5 space-y-4">
							<div>
								<label className="block text-xs font-bold text-cas-on-surface mb-1.5" htmlFor="incident-creator-input">
									Người tạo báo cáo
								</label>
								<input
							...
							<div>
								<label className="block text-xs font-bold text-cas-on-surface mb-1.5" htmlFor="incident-description-input">

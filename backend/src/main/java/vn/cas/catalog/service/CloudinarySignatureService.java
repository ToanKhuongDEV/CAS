...
public class CloudinarySignatureService {
  ...
  public CloudinarySignatureService(
      @Value("${cas.cloudinary.cloud-name:}") String cloudName,
  ...
  public UploadSignature sign(long storeId) {
    if (cloudName.isBlank() || apiKey.isBlank() || apiSecret.isBlank() || preset.isBlank())
      throw new ApiException(HttpStatus.SERVICE_UNAVAILABLE, ApiMessages.CLOUDINARY_NOT_CONFIGURED);
    ...
    try {
      ...
      return new UploadSignature(cloudName, apiKey, timestamp, folder, publicId, signature, preset);
    } catch (Exception e) {
  ...
  public void validateAsset(long storeId, String imageUrl, String storageKey) {
    ...
    try {
      ...
      String publicIdFromUrl =
          extensionIndex < 0 ? fileName : fileName.substring(0, extensionIndex);

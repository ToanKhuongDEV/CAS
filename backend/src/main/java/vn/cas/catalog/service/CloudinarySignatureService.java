...
public class CloudinarySignatureService {
    ...
    public UploadSignature sign(long storeId) {
        ...
        long timestamp=Instant.now().getEpochSecond(); String folder=rootFolder + "/" + storeId; String publicId=storeId + "_" + UUID.randomUUID();
        String value="folder="+folder+"&public_id="+publicId+"&timestamp="+timestamp+"&upload_preset="+preset+apiSecret;
    ...
    public void validateAsset(long storeId, String imageUrl, String storageKey) {
        if (imageUrl == null && storageKey == null) return;
        if (imageUrl == null || storageKey == null || cloudName.isBlank()) throw invalidImage();
        try {
            URI uri=URI.create(imageUrl);
            String path=uri.getPath();
            String fileName=path.substring(path.lastIndexOf('/') + 1);
            int extensionIndex=fileName.lastIndexOf('.');
            String publicIdFromUrl=extensionIndex < 0 ? fileName : fileName.substring(0,extensionIndex);
            if (!"https".equals(uri.getScheme()) || !"res.cloudinary.com".equals(uri.getHost())
                    ...
                    || !storageKey.equals(publicIdFromUrl)) throw invalidImage();
        } catch (IllegalArgumentException e) { throw invalidImage(); }
    }
    private ApiException invalidImage() { return new ApiException(HttpStatus.BAD_REQUEST,ApiMessages.INVALID_CATALOG_IMAGE); }

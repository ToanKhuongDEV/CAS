...
public class CatalogService {
  ...
  public CatalogService(
      CatalogMapper mapper, AuditLogService audit, CloudinarySignatureService cloudinary) {
    this.mapper = mapper;
    this.audit = audit;
  ...
  public void createCategory(
      OperationalPrincipal p, String n, String d, String t, int o, String s, UUID r) {
  ...
  public void updateCategory(
      OperationalPrincipal p, long id, String n, String d, int o, String s, UUID r) {
  ...
  public void createGroup(
      OperationalPrincipal p, String n, String t, int min, Integer max, int o, String s, UUID r) {
  ...
  public void createValue(
      ...
      String s,
      UUID r) {
    if (mapper.findOptionGroup(p.storeId(), groupId) == null) throw notFound();
  ...
  public void bulkStatus(OperationalPrincipal p, List<Long> ids, String status, UUID r) {
    if (ids.isEmpty()) throw new ApiException(HttpStatus.BAD_REQUEST, ApiMessages.INVALID_REQUEST);
  ...
  private void validItem(long storeId, MenuCommand c) {
    if (mapper.findCategory(storeId, c.categoryId()) == null) throw notFound();
    cloudinary.validateAsset(storeId, c.imageUrl(), c.imageStorageKey());
    for (Long tag : c.tagIds()) if (mapper.findTag(storeId, tag) == null) throw notFound();
    for (OptionGroupLink group : c.optionGroups())
      if (mapper.findOptionGroup(storeId, group.optionGroupId()) == null) throw notFound();
  }

  private void links(long storeId, long itemId, MenuCommand c) {
    for (Long tag : c.tagIds()) mapper.insertMenuItemTag(itemId, tag, storeId);
    for (OptionGroupLink group : c.optionGroups())
      mapper.insertMenuItemOptionGroup(
          itemId, group.optionGroupId(), storeId, group.displayOrder());
  }
  ...
  public record MenuCommand(
      ...
      int displayOrder,
      List<Long> tagIds,
      List<OptionGroupLink> optionGroups) {}
}

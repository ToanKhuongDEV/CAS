...
public class CatalogController {
    ...
    public ResponseEntity<ApiResponse<Void>> createCategory(
            @AuthenticationPrincipal OperationalPrincipal p, @Valid @RequestBody CategoryRequest b,
            HttpServletRequest r) {
        catalog.createCategory(p, b.name(), b.description(), b.categoryType(), b.displayOrder(),
                b.status(), rid(r));
        return created(r);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> updateCategory(
            ...
            @Valid @RequestBody UpdateCategoryRequest b, HttpServletRequest r) {
        catalog.updateCategory(p, id, b.name(), b.description(), b.displayOrder(), b.status(),
                rid(r));
        return ok(null, r);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> createTag(
            @AuthenticationPrincipal OperationalPrincipal p, @Valid @RequestBody TagRequest b,
            HttpServletRequest r) {
    ...
    public ResponseEntity<ApiResponse<Void>> updateTag(
            @AuthenticationPrincipal OperationalPrincipal p, @Positive @PathVariable long id,
            @Valid @RequestBody TagRequest b, HttpServletRequest r) {
    ...
    public ResponseEntity<ApiResponse<Void>> createGroup(
            ...
            @Valid @RequestBody OptionGroupRequest b, HttpServletRequest r) {
        catalog.createGroup(p, b.name(), b.selectionType(), b.minSelect(), b.maxSelect(),
                b.displayOrder(), b.status(), rid(r));
        return created(r);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> updateGroup(
            @AuthenticationPrincipal OperationalPrincipal p, @Positive @PathVariable long id,
            @Valid @RequestBody OptionGroupRequest b, HttpServletRequest r) {
        catalog.updateGroup(p, id, b.name(), b.selectionType(), b.minSelect(), b.maxSelect(),
                b.displayOrder(), b.status(), rid(r));
        return ok(null, r);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> createValue(
        ...
        catalog.createValue(p, id, b.name(), b.extraPrice(), b.isDefault(), b.displayOrder(),
                b.status(), rid(r));
        return created(r);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> createItem(
            @AuthenticationPrincipal OperationalPrincipal p, @Valid @RequestBody MenuItemRequest b,
            HttpServletRequest r) {
    ...
    public ResponseEntity<ApiResponse<Void>> updateItem(
            @AuthenticationPrincipal OperationalPrincipal p, @Positive @PathVariable long id,
            @Valid @RequestBody MenuItemRequest b, HttpServletRequest r) {
        catalog.updateItem(p, id, b.command(), rid(r));
        return ok(null, r);
    }

    @PatchMapping(ApiPaths.Catalog.ADMIN + "/items/bulk-status")
    public ResponseEntity<ApiResponse<Void>> bulk(@AuthenticationPrincipal OperationalPrincipal p,
    ...
    public record CategoryRequest(@NotBlank @Size(max = 150) String name,
            @Size(max = 65535) String description,
            @Pattern(regexp = "REGULAR|OPTION") String categoryType,
            @PositiveOrZero int displayOrder, @Pattern(regexp = "ACTIVE|INACTIVE") String status) {
    }

    public record UpdateCategoryRequest(@NotBlank @Size(max = 150) String name,
            @Size(max = 65535) String description, @PositiveOrZero int displayOrder,
            @Pattern(regexp = "ACTIVE|INACTIVE") String status) {
    }

    public record TagRequest(@NotBlank @Size(max = 150) String name,
            @Pattern(regexp = "ACTIVE|INACTIVE") String status) {
    }

    public record OptionGroupRequest(@NotBlank @Size(max = 150) String name,
            @Pattern(regexp = "SINGLE|MULTIPLE") String selectionType,
            @PositiveOrZero int minSelect, @PositiveOrZero Integer maxSelect,
            @PositiveOrZero int displayOrder, @Pattern(regexp = "ACTIVE|INACTIVE") String status) {
    }

    public record OptionValueRequest(@NotBlank @Size(max = 150) String name,
            @NotNull @DecimalMin("0.00") BigDecimal extraPrice, boolean isDefault,
            @PositiveOrZero int displayOrder, @Pattern(regexp = "ACTIVE|INACTIVE") String status) {
    }

    public record MenuItemRequest(@Positive long categoryId, @NotBlank @Size(max = 150) String name,
            @Size(max = 65535) String description, @NotNull @DecimalMin("0.00") BigDecimal price,
            @Size(max = 2048) String imageUrl, @Size(max = 512) String imageStorageKey,
            @Pattern(regexp = "ACTIVE|INACTIVE|SOLD_OUT") String availabilityStatus,
            @PositiveOrZero int displayOrder, List<@Positive Long> tagIds,
            List<@Valid OptionGroupLinkRequest> optionGroups) {
        CatalogService.MenuCommand command() {
            return new CatalogService.MenuCommand(categoryId, name, description, price, imageUrl,
                    imageStorageKey, availabilityStatus, displayOrder,
                    tagIds == null ? List.of() : tagIds,
                    optionGroups == null
                            ? List.of()
                            : optionGroups.stream()
                                    .map(x -> new CatalogService.OptionGroupLink(x.optionGroupId(),
                                            x.displayOrder()))
                                    .toList());
        }
    }

    public record OptionGroupLinkRequest(@Positive long optionGroupId,

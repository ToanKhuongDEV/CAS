...
public class OperatorManagementService {
  ...
  public Operator create(
      ...
      UUID requestId) {
    var firebaseUid =
        firebaseUserProvisioner.createUser(email, DEFAULT_OPERATOR_PASSWORD, displayName);
    var command =
        new CreateOperatorAccountCommand(
            principal.storeId(), firebaseUid, email, phone, displayName);
    try {

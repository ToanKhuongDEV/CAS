...
public class OperatorManagementController {
  ...
  public ResponseEntity<ApiResponse<OperatorResponse>> create(
    ...
    return ApiResponses.success(
        HttpStatus.CREATED,
        ApiMessages.OPERATOR_CREATED,
        new OperatorResponse(
            operator.id(), operator.firebaseUid(), operator.displayName(), operator.status()),
        request);
  }
  ...
  public ResponseEntity<ApiResponse<Void>> delete(
      ...
      HttpServletRequest request) {
    operatorService.deactivate(
        principal, operatorId, (java.util.UUID) request.getAttribute(RequestId.ATTRIBUTE_NAME));

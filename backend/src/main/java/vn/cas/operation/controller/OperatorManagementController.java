...
import vn.cas.operation.service.OperatorManagementService;
...
public class OperatorManagementController {
    ...
    public ResponseEntity<ApiResponse<OperatorResponse>> create(
        ...
        var operator = operatorService.create(principal, body.email(), body.phone(),
                body.displayName(),
        ...
        return ApiResponses.success(HttpStatus.CREATED, ApiMessages.OPERATOR_CREATED,
                new OperatorResponse(operator.id(), operator.firebaseUid(), operator.displayName(),
                        operator.status()),
                request);
    }
    ...
    public record CreateOperatorRequest(@NotBlank @Email @Size(max = 254) String email,
            @NotBlank @Size(max = 20) String phone, @NotBlank @Size(max = 150) String displayName) {
    }

...
public class CustomerTableSessionService {
    ...
    public CustomerTableSessionResolution resolveQr(CustomerTableSessionResolutionCommand command) {
        var tableSession = diningTableMapper.findTableSessionByActiveQrTokenForUpdate(command.qrToken());
        ...
        if (command.customerName() == null || command.customerName().isBlank()) {
            return new CustomerTableSessionResolution(
                    ResolutionStatus.CUSTOMER_INFORMATION_REQUIRED,
                    null,
                    tableSession.tableCode());
        }
    ...
    private long findOrCreateClientAccount(long storeId, CustomerTableSessionResolutionCommand command) {
        ...
        var createCommand = new CreateClientAccountCommand(storeId, command.customerName(), command.customerPhone());

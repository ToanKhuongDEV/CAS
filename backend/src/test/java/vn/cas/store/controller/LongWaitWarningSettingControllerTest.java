...
class LongWaitWarningSettingControllerTest {
    ...
    private final LongWaitWarningSettingService settingService = mock(
            LongWaitWarningSettingService.class);
    private final LongWaitWarningSettingController controller = new LongWaitWarningSettingController(
            settingService);
    ...
    void shouldReturnConfiguredLongWaitWarningMinutes() {
        ...
        assertThat(response.getBody().message())
                .isEqualTo(ApiMessages.LONG_WAIT_WARNING_SETTING_RETRIEVED);
        assertThat(response.getBody().data().longWaitWarningMinutes()).isEqualTo(25);
    }

    @Test
    void shouldReturnUpdatedLongWaitWarningMinutes() {
        when(settingService.update(any(), any(Integer.class), any()))
                .thenReturn(new LongWaitWarningSetting(0));
        var request = requestWithId();

        var response = controller.update(principal,
        ...
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().message())
                .isEqualTo(ApiMessages.LONG_WAIT_WARNING_SETTING_UPDATED);
